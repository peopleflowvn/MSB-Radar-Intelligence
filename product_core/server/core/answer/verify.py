# -*- coding: utf-8 -*-
"""Kiểm chứng bài viết của ⑤ — vòng tự sửa. Dùng chung mọi domain.

② đã có vòng nới khi truy hồi mỏng. ⑤ thì không có gì: nó viết ra một câu trả
...
**Vì sao không phụ thuộc nghiệp vụ:** cả bốn phép chỉ đọc `name` + `person_id`
của từng mục trong `chosen`, `sort_by`/`limit` của kế hoạch, và các số `[n]`
trong bài. Không phép nào cần biết đối tượng là ứng viên hay khách hàng — nên
`rb/answer` dùng lại y nguyên, và một lỗi tìm thấy ở đây được sửa cho cả hai.
"""
