# -*- coding: utf-8 -*-
"""Gieo route cho tác vụ mới `assistant_estimate_reasoning`.
...
TASK = "assistant_estimate_reasoning"
...
def gieo(apps, schema_editor):
    ...
    TaskModelRoute.objects.create(task=TASK, provider=provider, model=model,
                                  enabled=True, updated_by="migration 0022")


def go(apps, schema_editor):
    apps.get_model("ai", "TaskModelRoute").objects.filter(
        task=TASK, updated_by="migration 0022").delete()


class Migration(migrations.Migration):
    dependencies = [("ai", "0021_route_latency_optimized_answer")]
