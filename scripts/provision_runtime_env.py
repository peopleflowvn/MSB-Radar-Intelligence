...
def build_runtime_values(source: dict[str, str], current: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "GREENNODE_MODEL_EMBEDDING": current.get("GREENNODE_MODEL_EMBEDDING", ""),
        "HAYSTACK_TELEMETRY_ENABLED": "false",
    }


def write_runtime_env(source_path: Path, target_path: Path) -> None:
    values = build_runtime_values(_read(source_path), _read(target_path))
...
def main() -> None:
    ...
    write_runtime_env(args.source, args.target)
